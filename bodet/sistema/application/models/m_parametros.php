...
class m_parametros extends CI_Model{
	...
	//funcion listar tituladosfile
	function tituladosfile(){
		$this->db->select('*');
	...
	//funcion listar egresadosfile
	function egresadosfile(){
		$this->db->select('*');
	...
	//funcion listar cobranza
	function califile($ciclo){
		$this->db->select('*');
		$query = $this->db->where("ciclo_escolar", $ciclo);
